<template>
    <div class="h-[520px] lg:h-[1024px] w-full pointer-events-none" ref="container">
        ...
        <div class="w-full h-full relative" :style="'transform:translateY(' + translateY + 'px) scale(' + scale + ')'" >
            ...
            <NuxtImg  format="webp" sizes="1024px" :src="'/img/bg/mobile/'  + src" quality="100" class="lg:hidden object-cover w-full h-full" />
            <NuxtImg format="webp" sizes="md:1920px lg:2440" :src="'/img/bg/desktop/' + src" quality="100" class="max-lg:hidden object-cover w-full h-full" />
        </div>
    </div>
</template>
<script setup>
