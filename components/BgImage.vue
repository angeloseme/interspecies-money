<template>
    <div class="h-[520px] lg:h-[1024px] w-full pointer-events-none" ref="container">
        ...
        <div class="w-full h-full relative" :style="'transform:translateY(' + translateY + 'px) scale(' + scale + ')'" >
            ...
            <NuxtImg  format="webp" sizes="sm:1520px"  :src="'/img/bg/mobile/'  + src" class="lg:hidden object-cover w-full h-full" />
            <NuxtImg format="webp" sizes="sm:1520px md:2440px"  :src="'/img/bg/desktop/' + src" class="max-lg:hidden object-cover w-full h-full" />
        </div>
    </div>
</template>
<script setup>
